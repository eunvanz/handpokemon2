...
class BattleReady extends React.PureComponent {
  render () {
    const { onClickStart, auth, creditInfo, onClickDefense } = this.props
    const renderBody = () => {
      if (creditInfo.battleCredit > 0) {
        return (
          <div>
            <h4>시합을 시작할 준비가 됐나?<br />시합이 시작된 이후에 도망친다면 패배처리되니 조심하라구.</h4>
            <Button text='시합시작!' onClick={onClickStart} icon='fa fa-gamepad' />
            <Button className='m-l-5' text='수비배치' icon='fa fa-shield-check' color='orange' onClick={onClickDefense} />
          </div>
        )
      } else {
        return (
...
BattleReady.propTypes = {
  ...
  onClickDefense: PropTypes.func.isRequired
}
